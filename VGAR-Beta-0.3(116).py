...
###############################################################################
#B 114 - adds manual select slider!!                                          #
#Good results, so saving and debugging further in 115                         #
###############################################################################
#115 debug - Regression bug found, xex not loading                            #
#    False alarm - was just respecqt had wrong path to PROD  XEX not Dev one  #
#          A nice later feature would be a respecqt file editor to fix this   #
###############################################################################
#116 Leonado secret addition - as yet unused                                  #  
###############################################################################
...
#Extra globals for browse mode with slider:
...
start_image = os.path.join(APP_PATH, "atari-logo-trans.png")
...
logo_label = tk.Label(header_frame, image=logo_image, bg="grey")  # Display the resized logo
...
#heading_label.pack()  # Trying without label
...
def open_serial():
    ...
    try:
        ...
        time.sleep(2)
        #Leonado secret (remember to regression test effects on Uno R3
...
def enable_browse_controls():

    btn_left.config(state=tk.NORMAL)
    btn_right.config(state=tk.NORMAL)

  #  lbl_slider_mode.config(
  #      text="BROWSE MODE",
  #      fg="yellow"
    #)
def disable_browse_controls():

    btn_left.config(state=tk.DISABLED)
    btn_right.config(state=tk.DISABLED)

   # lbl_slider_mode.config(
   #     text="BATCH SIZE MODE",
   #     fg="white"
   # )
def toggle_select_mode():
    global browse_mode

    if not pre_loaded_filenames:
        return

    browse_mode = not browse_mode

    if browse_mode:

        btn_select.config(text="Confirm")

        enable_browse_controls()

        slider_update_from_position()

    else:

        btn_select.config(text="Select")

        confirm_current_selection()

        disable_browse_controls()
...
img = img.resize((300,225), Image.Resampling.LANCZOS)
photo = ImageTk.PhotoImage(img)
game_image_label.config(image=photo, relief="raised" , bg="black")
...
place(btn_Change_Filepath, "bottom_controls",dx=200,dy=-35)
...
place(btn_loadBatch, "bottom_controls", dx=300,dy=-35)

#Button for manual slider selection
btn_select = tk.Button(
    window,
    text="Select",
    command=toggle_select_mode
)
place(btn_select, "bottom_controls", dx=400,dy=-35)

def slider_update_from_position(event=None):
    global browse_index, current_selected_filename

    if not browse_mode:
        return

    slider_value = filename_count_slider.get()

    max_slider = 100

    batch_size = len(pre_loaded_filenames)

    browse_index = int(
        (slider_value / max_slider) * (batch_size - 1)
    )

    current_selected_filename = pre_loaded_filenames[browse_index]

    update_preview(current_selected_filename)
...
def move_selection_right():
    ...
    if browse_index < len(pre_loaded_filenames) - 1:
        browse_index += 1
...
filename_count_slider = tk.Scale(
    ...
    command=slider_update_from_position
)

btn_left = tk.Button(
    window,
    text="<",
    command=move_selection_left
)

btn_right = tk.Button(
    window,
    text=">",
    command=move_selection_right
)

state=tk.DISABLED


def apply_browse_index():
    global current_selected_filename
...
def update_slider_position():
    ...
    slider_position = int(
        (browse_index / (batch_size - 1)) * 100
    )

    filename_count_slider.set(slider_position)
...
def confirm_current_selection():

    global current_selected_filename
    global final_filename 
    if not current_selected_filename:
        return

    with open(final_file, "w") as file:
        file.write(current_selected_filename + "\n")

    pygame.mixer.music.load("cload.wav")
    pygame.mixer.music.play()

    print(current_selected_filename)
    final_filename=current_selected_filename



filename_count_slider.set(100)
#filename_count_slider.place(x=100,y=650)
place(filename_count_slider, "bottom_controls", dx=30)

place(btn_left,"bottom_controls",dx=10)
place(btn_right,"bottom_controls",dx=448)



slider_instructions =tk.Label(window, text="Batch Size:", font=("Arial", 12, "bold"))
place(slider_instructions, "bottom_controls", dy=-40, dx=40)


# Progressbar for showing progress during filename loading
progress = ttk.Progressbar(window, orient=tk.HORIZONTAL, length=150, mode='determinate')  # Proper initialization
progress.config(maximum=100)
#progress.place(x=100,y=633)
place(progress, "bottom_controls", dy=2,dx=250)
